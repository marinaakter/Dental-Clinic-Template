...
export const WhatsAppButton = () => {
  ...
  const whatsappInfo = clientInfo.whatsapp;
  const quickMessages = whatsappInfo.quickMessages;

  const handleQuickMessage = (message) => {
    const encodedMessage = encodeURIComponent(
      `${whatsappInfo.quickMessagePrefix} ${message}`
    );
    window.open(
      `https://wa.me/${whatsappInfo.whatsappNumber}?text=${encodedMessage}`,
      "_blank"
    );
    setIsOpen(false);
  };
  ...
  const whatsappStyle = {
    "--whatsapp-primary": whatsappInfo.accentColor,
    "--whatsapp-primary-hover": whatsappInfo.accentHoverColor
  };
    ...
    <div className="fixed bottom-6 right-6 z-40" data-testid="whatsapp-widget">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="p-4" style={{ backgroundColor: whatsappInfo.accentColor }}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-body font-semibold text-white text-sm">
                      {whatsappInfo.headerTitle}
                    </p>
                    <p className="font-body text-white/80 text-xs">
                      {whatsappInfo.headerSubtitle}
                    </p>
                  </div>
                </div>
                <button
            ...
            <div className="p-4 space-y-2 max-h-60 overflow-y-auto">
              <p className="font-body text-xs text-navy/50 uppercase tracking-wider mb-3">
                {whatsappInfo.quickMessagesTitle}
              </p>
            ...
            <div className="p-4 border-t border-gold/10">
              <button
                ...
                className="w-full text-white font-body font-medium py-3 rounded-xl transition-colors flex items-center justify-center gap-2"
                style={whatsappStyle}
                data-testid="start-whatsapp-chat-btn"
              >
                <MessageCircle size={18} />
                {whatsappInfo.startChatLabel}
              </button>
            </div>
          </motion.div>
        )}
      ...
      <motion.button
        ...
        className={`w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-colors ${
          isOpen ? "bg-navy" : "bg-[var(--whatsapp-primary)]"
        }`}
        style={whatsappStyle}
        data-testid="whatsapp-toggle-btn"
        aria-label={whatsappInfo.ariaLabel}
      >
      ...
      {!isOpen && (
        <span
          className="absolute bottom-0 right-0 w-14 h-14 rounded-full animate-ping opacity-30"
          style={{ backgroundColor: whatsappInfo.accentColor }}
        />
      )}
    </div>
  );
